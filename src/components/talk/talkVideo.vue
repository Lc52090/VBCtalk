      ...
      <div class="v-container">
          ...
          <div class="v-topnews">
            <div class="topnews-left">
              ...
              <div class="top-text">
                <div class="text-top">
                  <span>回放</span>
                  <span>{{talkMainList.title}}</span>
                  <span>2020-05-15 15:00-18:00</span>
                </div>
                <div class="text-bottom">
                  <span>主办方:{{talkMainList.zhubanfang}}</span>
                  <div class="border">
                    <span>+关注</span>
                    <span>153256</span>
                  </div>
            ...
            <div class="topnews-right">
              <span>
                <i><img
                    ...
                    alt=""
                  ></i>
                <span>{{talkMainList.view_count}}</span>
              </span>
              <span>
                <i><img
        ...
        <div class="v-produce">
          ...
          <div class="v-text">
            <!-- 简介 -->
            <div v-if="selected===0">
              <p>{{talkMainList.jianjie}}</p>
              <div class="v-img">
                <img
                  :src="talkMainList.pic1"
                  alt=""
                >
              </div>
            </div>
            ...
            >
              <ul>
                <li
                  v-for="(item,index) in talkMainList.luntan_live_video_array"
                  :key="index"
...
export default {
  ...
  created() {
    this.gettalkMainList()
  },
  methods: {
    gettalkMainList() {
      this.id = this.$route.query.id
      let params = Qs.stringify({ id: this.id })
      this.$http.post('api_get_luntan_video', params).then(response => {
        const res = response.data
        this.talkMainList = res
        console.log(res)
      })
    },
    // 点解文本切换selected
    ...
    DoCollet() {
      ...
      let params = Qs.stringify({
        ...
        live_video_id: this.id // 红人视频id
      })
      return params
    },
    // 点赞接口
    async handleClickCount() {
      const { data: res } = await this.$http.post('luntan_live_video_do_zan', this.DoPlus())
      this.talkMainList.like_count++
    },
    // 获取收藏列表
    async getfavoriteList() {
      ...
      for (let i = 0; i < this.favorateList.length; i++) {
        console.log(this.favorateList[i].id.indexOf(this.id))
        if (this.favorateList[i].id.indexOf(this.id) !== -1) {
    ...
    async handleClickFavorite() {
      ...
      const { data: res } = await this.$http.post('api_luntan_live_video_do_favorite', this.DoCollet())
      this.$message.success('收藏成功')
      this.talkMainList.favorite_count++
    },
    // 点击往期回顾视频切换视频
    async changeVideo(item, index) {
      let params = Qs.stringify({ id: item.id })
      console.log(item)
      const { data: res } = await this.$http.post('api_get_luntan_video', params)
      this.talkMainList = res
    }
  }
}
